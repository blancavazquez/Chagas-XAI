...
Code for training MTS models using synthetic oversampling.
"""
...
from models.vae import VAE, data_augmentation_method, Dataset
...
latent_dim_vae = 16
batch_size_vae = 25
patience_vae = patience 
""" -----------------------------------------------------"""

def plot_ecg_explainability(sig_tensor, lead_names,title='ECG Sample (400 HZ)'):
    """
    Input: las señales deben ser de tamaño (12, 4000)
    Para este paso, todas las señales están en una misma frecuencia (400)
    """
    SAMPLING_RATE = 400 
...
def loading_signals(path, folder, time_steps, database):
    ...
    for file in files: #Extract signals for each file
        ...
        if database == "ptbxl":
            df_ecg = pd.DataFrame(signals_transpose, columns = lead_names_ptb) 
        ...
        DF_per_patient = np.concatenate((df_ecg, DF_per_patient), axis=0) #(3, 12, 5000)
        i=i+1
...
def concat_datasets(xptb, xsamitrop, yptb, ysamitrop,):
    x = np.concatenate((xptb, xsamitrop), axis=0) #(3, 12, 5000)
    y = np.concatenate((yptb, ysamitrop), axis=0)
    print("x: ", x.shape, "y: ", y.shape)
    return x, y 

def concat_datasets_augmented(xptb, xsamitrop, xsamitrop_augmented, yptb, ysamitrop,yvae):
    print("xptb: ", xptb.shape, "xsamitrop: ", xsamitrop.shape, "xsamitrop_augmented: ", xsamitrop_augmented.shape)
    xtrain = np.concatenate((xptb, xsamitrop, xsamitrop_augmented), axis=0) #(3, 12, 5000)
    ytrain = np.concatenate((yptb, ysamitrop, yvae), axis=0)
    print("xtrain (final): ", xtrain.shape, "ytrain (final): ", ytrain.shape)
...
def plot_augmented_data():
    ...
    features = pd.concat([orig_df.drop('Dataset', axis=1), aug_df.drop('Dataset', axis=1)])
    labels = ['Original']*len(orig_df) + ['Augmented']*len(aug_df)

    pca_results = PCA(n_components=2).fit_transform(features)

    sns.scatterplot(x=pca_results[:,0], y=pca_results[:,1], hue=labels, alpha=0.5)
    plt.title("PCA Projection: Original vs Augmented Space")
    plt.show()

def augmentation_vae(dataset_for_training, dataset_for_validating, num_features, latent_dim_vae, augmentation_rate, num_leads_dict, feature_selection):
    print("train:", dataset_for_training["X"].shape)
    print("val:", dataset_for_validating["X"].shape)

    train_data = Dataset(dataset_for_training["X"], dataset_for_training["y"])
    val_data = Dataset(dataset_for_validating["X"], dataset_for_validating["y"])
    train_loader = DataLoader(train_data, batch_size=batch_size_vae,shuffle=True, num_workers = 3, pin_memory=True,)
    val_loader = DataLoader(val_data, batch_size=batch_size_vae,shuffle=True, num_workers = 3, pin_memory=True,)

    print("\n ------------------ VAE model --------------------------")
    model = VAE(num_features = num_features, latent_dim = latent_dim_vae)
    model.to(device)
    logger = TensorBoardLogger(save_dir='../metrics/') 
    es_callback = EarlyStopping(patience=patience_vae, verbose=1, monitor='val_loss', mode='min')
    checkpoint_callback = ModelCheckpoint(monitor='val_loss',mode="min",dirpath='../models_vae/',filename='_{epoch:02d}-{val_loss:.4f}',)    
    trainer = Trainer(max_epochs=epochs_vae,logger = True,
    ...
    print("\n***** Time for training:", datetime.datetime.now() - start_time, " *****")
    
    print("\n ------------------ Starting augmenting data ---------------------")
    new_data = np.zeros((1,dataset_for_training["X"].shape[2],dataset_for_training["X"].shape[1]))
    for rate in range (augmentation_rate):
        temp = data_augmentation_method(model, train_loader, device, dataset_for_training["X"].shape[1], rate, feature_selection)
        new_data = np.concatenate((temp, new_data), axis=0)    
    new_data = new_data[:-1]

    new_data = new_data.transpose(0, 2, 1) 
    y_label = np.full(shape=new_data.shape[0], fill_value=1) 

    return new_data, y_label


print("\n\n*************************************")
print("*1) Loading and processing(PTBXL)* ")
database = "ptbxl"
path_ptb = "../data/PTB_Aceptadas_500/Split/"
...
print("\n\n*************************************")
print("*3) Data augmentation (only train data in SAMITROP)* ")
num_features = xtrain_samitrop.shape[1] * xtrain_samitrop.shape[2]
...
data_vae = { # assemble the final processed data into a dictionary
        "n_classes": 2,
        "n_steps": num_leads_dict,
        "n_features": 4000,
        "train_X": xtrain_samitrop,
        "train_y": ytrain_samitrop,
        "val_X": xval_samitrop,
        "val_y": yval_samitrop,
    }
dataset_for_training = {"X": data_vae['train_X'],"y": data_vae['train_y'],} #(batchsize, leads, samples) (batchsize,)
dataset_for_validating = {"X": data_vae['val_X'],"y": data_vae['val_y'],} #(batchsize, leads, samples) (batchsize,)
xtrain_samitrop_augmented,  ytrain_samitrop_augmented = augmentation_vae(dataset_for_training, dataset_for_validating, 
                                                                         num_features, latent_dim_vae, augmentation_rate, 
                                                                         num_leads_dict, feature_selection)

print("\n\n*************************************")
print("*4) Concat (PTB + SAMITROP + Augmented data)* ")

print("ptb:", xtrain_ptb.shape)
print("samitrop:", xtrain_samitrop.shape)
print("vae:", xtrain_samitrop_augmented.shape)

xtrain, ytrain = concat_datasets_augmented(xtrain_ptb, xtrain_samitrop, xtrain_samitrop_augmented, ytrain_ptb, ytrain_samitrop, ytrain_samitrop_augmented) #(batchsize, leads, 4000) (batchsize,)
