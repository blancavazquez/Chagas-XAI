...
Code for training MTS models using the original imbalanced dataset
without applying any class balancing technique.
"""
import gc
import os
import torch
...
def loading_signals(path, folder, time_steps, database):
    ...
    for file in files: #Extract signals for each file
        ...
        if database == "ptbxl":
            ...
            df_ecg = pd.DataFrame(signals_transpose, columns = lead_names)

        df_ecg = df_ecg.to_numpy().T 
        df_ecg = df_ecg[np.newaxis, :, :] 
...
def preprocess(signals, fs_orig, fs_target, database):
    print("Before processing (original): ", signals.shape) #(num_patients, leads, amplitud)
    ...
    for id_pat in range(signals.shape[0]):
        ...
        DF_all_patients = np.concatenate((DF_per_patient_numpy, DF_all_patients), axis=0) 
    x = DF_all_patients[:-1]
    print("x: ", x.shape)
    return x

def concat_datasets(xptb, xsamitrop, yptb, ysamitrop,):
    xtrain = np.concatenate((xptb, xsamitrop), axis=0)
...
class Dataset(Dataset):
  ...
  def __getitem__(self, i):
    target_in_true = torch.tensor(self.xtrue[i], dtype=torch.float) 
    target_out = torch.tensor(self.label[i], dtype=torch.int)
